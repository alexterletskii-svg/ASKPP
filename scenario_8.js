(function initScenarioEngine() {
    ...
    const steps = [
        ...
        // --- 3. ИСПОЛЬЗОВАНИЕ ОКНА (ВТОРОЙ СПОСОБ – СПИСОК С СУММАМИ) ---
        {
            ...
            // ИСПРАВЛЕНИЕ: Добавлен слушатель клавиатуры
            ...
            <b>2.</b> Нажмите <span class="action-badge">Ok</span> (или клавишу <b>Enter</b>) для применения.`,
            validate: (e) => {
                // ИСПРАВЛЕНИЕ: Проверяем нажатие Enter
        ...
        // --- 5. ВЫКЛЮЧЕНО / ВОССТАНОВЛЕНИЕ ---
        {
            ...
            text: () => {
                ...
                if (el && el.classList.contains('inactive')) {
                    ...
                    return `Группа успешно активирована.<br><br>Кликните по ней еще раз для перехода к следующему шагу.`;
                }
            },
            validate: (e) => {
                const el = e.target.closest('.filter-block');
        ...
        // --- 8. ПОДТВЕРЖДЕНИЕ, ЧТО ОКНО ТО ЖЕ САМОЕ ---
        {
            ...
            Нажмите любую кнопку (или нажмите клавишу <b>Enter</b>), чтобы закрыть окно и завершить обучение.`,
            validate: (e) => {
                // ИСПРАВЛЕНИЕ: Проверяем нажатие Enter или Escape
                if (e.type === 'keydown' && (e.key === 'Enter' || e.key === 'Escape')) return true;
