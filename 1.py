...
# convert_paths.py
input_file = "file_paths.txt"        # исходный список файлов (только имена или относительные пути)
output_file = "massiv s roi.txt"    # JS‑массив для вставки в HTML
prefix = "picture"                  # папка, которую добавляем в начало каждого пути
...
with open(output_file, "w", encoding="utf-8") as f:
    f.write("const allFilePaths = [\n")
    for i, path in enumerate(paths):
        # Собираем полный путь: picture/ + оригинальное имя
        full_path = f"{prefix}{path}"
        # Экранируем кавычки, если они вдруг есть в названии файла
