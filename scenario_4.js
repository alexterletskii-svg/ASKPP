// --- ДВИЖОК ОБУЧАЮЩИХ СЦЕНАРИЕВ: СЦЕНАРИЙ №5 ---
(function initScenarioEngine() {
    ...
    const steps = [
        // --- 1. ВЫБОР РУЛОНА ---
        {
            ...
            eventType: 'dblclick',
            placement: 'right',
            text: 'Добро пожаловать в обучение!<br><br>Сегодня мы изучим новую цветовую индикацию статусов камер. Для начала дважды кликните по <span class="action-badge">ЛЮБОМУ</span> рулону.',
            purpose: 'Выбор рулона подгрузит его карту дефектов, чтобы мы могли анализировать данные и фильтровать информацию.',
            validate: (e) => {
        ...
        // --- 3. ВКЛЮЧЕНИЕ ОДНОЙ КАМЕРЫ ---
        {
            ...
            eventType: 'click',
            placement: 'bottom',
            text: 'Видите? Теперь все кнопки статусов (справа) стали красными.<br><br>А теперь кликните по камере <span class="action-badge">01 Низ LF</span>, чтобы включить только её.',
            purpose: 'Мы включим одну левую фронтальную камеру и посмотрим, как изменится глобальный статус группы "Низ LF".',
            validate: (e) => {
        ...
        // --- 4. ВКЛЮЧЕНИЕ ВТОРОЙ КАМЕРЫ (ДЕМОНСТРАЦИЯ ЖЁЛТОГО ЦВЕТА) ---
        {
            delay: 500,
            targetSelector: '#content-cameras',
            eventType: 'click',
            placement: 'bottom',
            text: 'Посмотрите на кнопку группы <b>Низ LF</b> (справа от камер). Она стала <b style="color:#ffda44;">жёлтой</b>! Это значит, что включена лишь часть камер из этой зоны.<br><br>Теперь кликните по камере <span class="action-badge">02 Низ LF</span>.',
            purpose: 'Включив оставшуюся камеру из этой группы, мы переведем её статус в полностью активный.',
            validate: (e) => {
                const target = e.target.closest('.filter-block');
                if (!target) return false;
                const cameraName = target.innerText.trim();
        ...
        // --- 5. ВКЛЮЧЕНИЕ ГРУППЫ КАМЕР (ДЕМОНСТРАЦИЯ ЗЕЛЁНОГО ЦВЕТА) ---
        {
            delay: 500,
            targetSelector: '#content-cameras',
            eventType: 'click',
            placement: 'bottom',
            text: 'Отлично! Кнопка "Низ LF" стала <b style="color:#00ff00;">зелёной</b> (все камеры зоны активны).<br><br>Цветовая индикация работает и как кнопка переключения! Нажмите на красную кнопку <span class="action-badge">Верх LF</span>.',
            purpose: 'Клик по кнопке статуса быстро включает сразу все её камеры. Кнопка "Верх LF" моментально станет зелёной.',
            validate: (e) => {
                const target = e.target.closest('.filter-block');
                if (!target) return false;
        ...
        // --- 6. ПРОВЕРКА РЕЗУЛЬТАТА (КЛИКИ ПО ДЕФЕКТАМ) ---
        {
            ...
            text: () => `На карте теперь отображаются только те дефекты, которые зафиксировали включённые камеры.<br><br>Кликните по <b>2 любым дефектам</b> на карте, чтобы посмотреть их детали.<br><br>Осталось кликнуть: <span class="action-badge">${2 - window.tutCameraDefectClicks}</span>`,
            purpose: 'Это позволяет быстро локализовать проблему и анализировать качество конкретных сторон или секций рулона.',
            validate: (e) => {
