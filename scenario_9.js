...
(function initScenarioEngine() {
    ...
    const steps = [
        ...
        // --- 2. ПЕРЕХОД В РЕАЛЬНЫЙ РЕЖИМ ---
        {
            ...
            Перейдите в режим <span class="action-badge">реальный</span>, нажав на соответствующую кнопку в панели снизу.`,
            validate: (e) => !!e.target.closest('#btn-real')
        },

        // --- 3. КОНЦЕПЦИЯ И ВКЛЮЧЕНИЕ "Ч.ДЕФЕКТЫ" ---
        {
            delay: 1000,
            onEnter: () => {
            ...
            placement: 'top',
            text: `<div class="purpose-text">
                Карта рулона изначально показывает <b>объединенные</b> дефекты. Но пользователь может визуализировать каждую составную часть по отдельности.
            </div>
            Нажмите на кнопку <span class="action-badge">Ч.дефекты</span> в нижней панели.<br><br>Обратите внимание: при её включении красные полосы распадаются на маленькие крестики (+).`,
            validate: (e) => !!e.target.closest('#tut-btn-partial-defects')
        },

        // --- 4. ИЗУЧЕНИЕ ОТКРЫВШИХСЯ ДЕФЕКТОВ НА КАРТЕ ---
        {
            delay: 600,
            ...
            <div class="purpose-text">
                Крупные объекты разбились на мельчайшие базовые точки (+), что даёт возможность изучить их по одному.
            </div>
            Кликните по <b>2 любым крестикам (+)</b> на карте.<br><br>
        ...
        // --- 5. ВЫКЛЮЧЕНИЕ "Ч.ДЕФЕКТЫ" ---
        {
            delay: 600,
            targetSelector: '#tut-btn-partial-defects',
            eventType: 'click',
            placement: 'top',
            text: `Режим детальной разбивки дает максимум информации для технолога, однако при быстром аудите множественные мелкие элементы могут визуально перегружать картину.<br><br>
            Поэтому после изучения деталей рекомендуется возвращать этот режим в исходное состояние.<br><br>
            Нажмите кнопку <span class="action-badge">Ч.дефекты</span> ещё раз, чтобы <b>выключить</b> его.`,
            validate: (e) => !!e.target.closest('#tut-btn-partial-defects')
        }
    ];
    ...
    function finishScenario() {
        ...
        body.innerHTML = '<b>Сценарий #4.1 успешно завершён!</b><br><br>Теперь вы умеете переключаться между базовым и детализированным отображением информации. Использование режима <b>Реальный</b> в связке с кнопкой <b>Ч.дефекты</b> особенно полезно для изучения составных объектов.';
