...
(function initScenarioEngine() {
    ...
    const steps = [
        // --- 1. ВЫБОР РУЛОНА ---
        {
            ...
            eventType: 'dblclick',
            placement: 'right',
            text: 'Добро пожаловать в обучение по детальному осмотру!<br><br>Сперва необходимо загрузить данные рулона в рабочую область.<br>Кликните по <span class="action-badge">ЛЮБОМУ</span> рулону в таблице слева 2 раза.',
            validate: (e) => {
        ...
        // --- 2. РЕЖИМ БОЛЬШОГО ИЗОБРАЖЕНИЯ ---
        {
            ...
            onEnter: () => {
                document.querySelectorAll('.sb-panel').forEach(b => {
                    ...
                    if (txt === 'Rol' || txt === 'Roi') b.id = 'tut-btn-rol';
                });
            },
        ...
        // --- 9. ЗАКРЕПЛЕНИЕ ROI: ОТКЛЮЧИТЬ ЧЕРЕЗ SHIFT (ПЕРВОЕ НАЖАТИЕ) ---
        {
            ...
            eventType: 'keydown',
            placement: 'bottom-right',
            text: 'Вместо клика по кнопке, ROI можно быстро скрывать и показывать с помощью клавиатуры.<br><br>Нажмите клавишу <span class="action-badge">Shift</span>, чтобы <b>СКРЫТЬ</b> красную рамку.',
    ...
    function finishScenario() {
        tooltip.style.display = 'none';
        activeTarget = null;
        showCustomAlert('<b>Сценарий #6 успешно завершён!</b><br><br>Вы научились переключаться в режим детального просмотра дефекта, изменять масштаб картинки, использовать быструю навигацию клавиатурой, а также быстро управлять видимостью рамок <b>ROI</b> при помощи мыши и клавиши Shift.');
    }
